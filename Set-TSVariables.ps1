...
Try {
	...
	[string]$appScriptDate = 'YYYY.MM.DD'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		if(get-eventlog -LogName System -InstanceId 2147484722 -after (Get-date).addDays(-1)){
			$TSEnv.Value("RestartScheduled") = "True"
			Write-Output "Restart is already scheduled. No need to proceed."
		}else{
			$TSEnv.Value("RestartScheduled") = "False"
			Write-Output "No Pending Reboots Detected."
		}
